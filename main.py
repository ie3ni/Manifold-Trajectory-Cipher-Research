...
# ============================================================
# NONLINEAR MANIFOLD CIPHER ENGINE
# BLAKE2b-FREE VERSION
# ============================================================
...
class PureManifoldCipher:
    ...
    def _prime_field_round(self, absorbed_byte: int):
        x_inv = mod_inverse(self.x + absorbed_byte + self.counter + 1, self.MOD_X)
        y_inv = mod_inverse(self.y + self.x + absorbed_byte + 1, self.MOD_Y)
        z_inv = mod_inverse(self.z + self.y + absorbed_byte + 1, self.MOD_Z)
        w_inv = mod_inverse(self.w + self.z + absorbed_byte + 1, self.MOD_W)
...
if __name__ == "__main__":
    print("=" * 72)
    print("        PURE MANIFOLD TRAJECTORY CIPHER CONSOLE")
    print("=" * 72)
    ...
    if not message:
        message = "Default pure manifold deployment test payload."
        print(f"[*] Empty input detected. Using default: '{message}'")

    print("\n[*] Initializing pure nonlinear manifold orbit...")
    print("[*] Applying prime-field, ARX, cross-coupling, and permutation rounds...")
    print("[*] BLAKE2b removed from keystream and manifold diffusion path.")
